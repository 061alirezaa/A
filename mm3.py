...
def main_kb(lang):
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(t["deal"], callback_data="new_deal")],
        [InlineKeyboardButton(t["admins"], callback_data="list_admins")],
...
def owner_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton("🌐 تغییر وضعیت Join اجباری", callback_data="toggle_join")],
        [InlineKeyboardButton("✏️ تغییر کانال Join", callback_data="change_join_channel")]
    ])
...
async def start(message: Message):
    ...
    if not allowed:
        ...
        return
    async with aiosqlite.connect(DB_NAME) as db:
        lang = await (await db.execute("SELECT language FROM users WHERE telegram_id=?", (message.from_user.id,))).fetchone()
        lang = lang[0]
...
async def stats(call: CallbackQuery):
    ...
    await call.message.answer(f"👥 Users: {users[0]}\n📁 Deals: {deals[0]}")
...
async def finish_deal(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if call.from_user.id != deal[6]:  # فقط واسطه
