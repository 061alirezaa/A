...
async def add_user(user):
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            ...
            (
                user.id,
                user.username if user.username else f"id_{user.id}",
                "owner" if user.id == OWNER_ID else "user",
                0,
                0
            )
        )
...
def finish_kb(deal_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
...
def rating_kb(deal_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text=f"⭐{i}", callback_data=f"rate:{deal_id}:{i}")
...
def owner_panel_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="➕ افزودن واسطه", callback_data="add_admin")],
            [InlineKeyboardButton(text="➖ حذف واسطه", callback_data="remove_admin")]
        ]
    )
...
async def deal_counterparty(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        cur = await db.execute(
            ...
            (username, username if username.isdigit() else -1)
        )
...
async def add_admin_cb(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != OWNER_ID:
        await call.answer("⛔ فقط مالک می‌تواند این کار را انجام دهد", show_alert=True)
        return
    await state.set_state(OwnerState.add_admin)
    await call.message.answer("📝 لطفاً آیدی یا یوزرنیم کاربر را برای افزودن به واسطه وارد کنید:")


@dp.callback_query(F.data == "remove_admin")
async def remove_admin_cb(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != OWNER_ID:
        await call.answer("⛔ فقط مالک می‌تواند این کار را انجام دهد", show_alert=True)
        return
    await state.set_state(OwnerState.remove_admin)
    await call.message.answer("📝 لطفاً آیدی یا یوزرنیم واسطه‌ای که می‌خواهید حذف کنید وارد کنید:")


# ================== PROCESS OWNER STATE MESSAGES ==================
@dp.message(OwnerState.add_admin)
async def add_admin_process(message: Message, state: FSMContext):
    user_input = message.text.replace("@", "")
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute(
            "SELECT telegram_id, username FROM users WHERE username=? OR telegram_id=?",
        ...
        await db.commit()
    await message.answer(f"✅ کاربر {row[1]} به عنوان واسطه اضافه شد.")
    await state.clear()


@dp.message(OwnerState.remove_admin)
async def remove_admin_process(message: Message, state: FSMContext):
    user_input = message.text.replace("@", "")
    async with aiosqlite.connect(DB_NAME) as db:
        row = await (await db.execute(
            "SELECT telegram_id, username, role FROM users WHERE username=? OR telegram_id=?",
            (user_input, user_input)
        )).fetchone()
        if not row:
            await message.answer("❌ کاربر یافت نشد.")
            ...
            return
        if row[2] != "admin":
            await message.answer("❌ این کاربر واسطه نیست.")
            await state.clear()
            return
        await db.execute(
        ...
        await db.commit()
    await message.answer(f"✅ کاربر {row[1]} از واسطه حذف شد.")
    await state.clear()
...
async def send_stats_file(call: CallbackQuery):
    if call.from_user.id != OWNER_ID:
        await call.answer("⛔ فقط مالک می‌تواند مشاهده کند", show_alert=True)
        return

    async with aiosqlite.connect(DB_NAME) as db:
        # همه کاربران
        all_users = await (await db.execute("SELECT username FROM users")).fetchall()
        # تعداد معاملات
        deals = await (await db.execute("SELECT COUNT(*) FROM deals")).fetchone()
        # واسطه‌ها و امتیاز
        admins = await (await db.execute(
            "SELECT username, rating_avg, rating_count FROM users WHERE role='admin'"
        )).fetchall()
    ...
    text_lines = [
        ...
        f"📁 تعداد معاملات: {deals[0]}",
        "",
        "📝 لیست کاربران:"
    ]

    for idx, user in enumerate(all_users, start=1):
        username = user[0] if user[0] else "بدون یوزرنیم"
        text_lines.append(f"{idx}- (@{username})")

    text_lines.append("\n👑 لیست واسطه‌ها و امتیازها:")
    if admins:
        for admin in admins:
            username, avg, count = admin
            text_lines.append(f"- @{username} | امتیاز: {avg:.2f} | تعداد رای: {count}")
    else:
        text_lines.append("هیچ واسطه‌ای وجود ندارد.")

    # ساخت فایل در حافظه
    file_content = "\n".join(text_lines)
    stats_file = io.BytesIO()
    stats_file.write(file_content.encode("utf-8"))
    stats_file.seek(0)
